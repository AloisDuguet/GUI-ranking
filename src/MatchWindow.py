...
class MatchWindow(GroupWindow):
    def __init__(self, root, listToRank, nameWindow):
        ...
        self.initLowerFrame(MatchElement)

    def initLowerFrame(self, element):
        self.lowerFrame = ttk.Frame(self.root,
                                    relief='raised')
        self.nElements = len(self.listToRank.competitors)
        self.elements = []

        if len(self.listToRank.competitors) != 2:
            raise("MatchWindow was coded for 1v1 matches, "
            "cf self.winner, self.loser, self.qualifyButton...")

        for i in range(self.nElements):
            self.elements.append(element(self.lowerFrame, self.listToRank.competitors[i], self.qualifyButton))
    ...
    def qualifyButton(self, matchElement):
        ...
        self.winner = self.listToRank.competitors[indexClick]
        if indexClick == 0:
            self.loser = self.listToRank.competitors[1]
        else:
            self.loser = self.listToRank.competitors[0]
        self.root.quit()
